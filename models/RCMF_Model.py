...
from models.encoder_decoder.RCMF_encoder import Encoder
from models.encoder_decoder.RCMF_decoMFR import Decoder
...
class RCMF(BasicModel):
    ...
    def forward(self, **kwargs):
        ...
        if(xe_rl):
            decoder_out, scores = self.decoder(gx, seq, encoder_out, seq_mask, att_mask)
            #return F.log_softmax(decoder_out, dim=-1), F.log_softmax(scores, dim=-1)
            return F.log_softmax(decoder_out, dim=-1),scores
        else:
    ...
    def get_logprobs_state(self, **kwargs):
        ...
        xe_rl = cfg.SOLVER.XE_RL  # 控制参数，后续需改为全局参数
        if(xe_rl):
            decoder_out0, scores = self.decoder(gx, ys[:, -1].unsqueeze(-1), encoder_out, seq_mask, att_mask)
            decoder_out = decoder_out0.squeeze(1)
        else:
            decoder_out = self.decoder(gx, ys[:, -1].unsqueeze(-1), encoder_out, seq_mask, att_mask).squeeze(1)
        #############---------------------------################################
...
class RCMF_Base(RCMF):
    def __init__(self):
        super(RCMF_Base, self).__init__()
        self.vocab_size = cfg.MODEL.VOCAB_SIZE + 1
...
# 采用 Base SwinTransformer
# pre-trained on Regular ImageNet-1K
class RCMF_Base_22K(RCMF):
    def __init__(self):
        super(RCMF_Base_22K, self).__init__()
        self.vocab_size = cfg.MODEL.VOCAB_SIZE + 1
