...
class ResidualAttentionBlock(nn.Module):
    ...
    def attention(self, x: torch.Tensor):
        mask = torch.empty(x.size(0), x.size(0))
...
class Decoder(nn.Module):
    def __init__(
        ...
        ###################——————————————————————————————###########################################
        ## MFR分支
        self.ln_final = LayerNorm(self.embed_dim)
        ...
        self.text_projection = nn.Parameter(torch.empty(self.embed_dim, embed_dim))
    ...
    def encode_text(self, text):
        x = self.word_embed(text)  # [batch_size, n_ctx, d_model]
        positional_embedding = nn.Parameter(torch.zeros(x.size(1), self.embed_dim)).cuda()
    ...
    def forward(self, gx, seq, encoder_out, seq_mask=None, att_mask=None):
        ...
        ############################——————————————————————————————###########################################
        xe_rl = cfg.SOLVER.XE_RL  # 控制参数，后续需改为全局参数
        if(xe_rl):
            ###################——————————————————————————————###########################################
            ## MFR分支
            mlm_feats = self.encode_text(seq)
            mlm = self.cross_former(mlm_feats,encoder_out, encoder_out)

            mlm = self.mlm_head(mlm)  # [batch_size, text_len, num_colors] Mask decoding head

            scores = mlm.float().reshape(-1, self.vocab_size)
            #print("scores:",scores)
            ###################——————————————————————————————###########################################
            for layer in self.layers:
...
class DecoderLayer(nn.Module):
    ...
    def forward(self, gx, x, encoder_out, seq_mask, att_mask=None):
        ...
        if self.use_gx:
            ...
            #print("x_cat size: ",x_cat.shape)
            x_cat = self.fuse_layer(x_cat) + x
            x_cat = self.ff_layer(x_cat)
            x_cat = self.dropout(x_cat)
            x = self.layer_norm2(x_cat + x_cat)
            #############----------------------################################
        short_cut = x
