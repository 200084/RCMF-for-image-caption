...
class EncoderLayer(nn.Module):
    ...
    def __init__(
        ...
        #############----------------------################################
        # 构造注意力核心操作层
        self.FocusedLinearAttention = FocusedLinearAttention(
                embed_dim, window_size=to_2tuple(self.window_size), num_heads=num_heads,
                qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.,
                #focusing_factor=3, kernel_size=1)
    ...
    def forward(self, x, att_mask=None):
        ...
        if self.use_gx:
            ...
            attn_windows_w = self.FocusedLinearAttention(x_windows, mask=_mask)
            attn_windows_gx = self.SMA(gx)
            attn_windows = torch.cat([attn_windows_w, attn_windows_gx], 1)   
            #attn_windows = self.o_linear(attn_windows)
                
        else:
            _mask = self.attn_mask
            attn_windows = self.FocusedLinearAttention(x_windows, mask=_mask)
            #######################---------------------------#########################            
