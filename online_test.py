...
class Tester(object):
    def __init__(self, args):
        ...
        self.setup_network()
        self.onlineTester = OnlineTester(
            #eval_ids = cfg.DATA_LOADER.TEST_4W_ID,
            eval_ids = cfg.DATA_LOADER.VAL_4W_ID,
    ...
    def eval(self, epoch):
        #res = self.onlineTester(self.model, 'test1_' + str(epoch))
        res = self.onlineTester(self.model, 'val1_' + str(epoch))

    def snapshot_path(self, name, epoch):
        snapshot_folder = os.path.join(cfg.ROOT_DIR, 'snapshot-mlmEnsamble7')
