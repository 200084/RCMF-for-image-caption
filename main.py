...
class Trainer(object):
    def __init__(self, args):
        ...
        #self.p = 0.01
        self.p = 0.001
        self.criterion1 = nn.CrossEntropyLoss(ignore_index=-1)
        ###############——————————————————————————————############
    ...
    def snapshot_path(self, name, epoch):
        ...
        snapshot_folder = os.path.join(cfg.ROOT_DIR, 'snapshot-mlm-l3')
    ...
    def forward(self, kwargs):
        if self.rl_stage == False:
            ###################——————————————————————————————#######################
            # XE训练过程损失计算
            logit,scores = self.model(**kwargs)
            
            loss, loss_info = self.xe_criterion(logit, kwargs[cfg.PARAM.TARGET_SENT])
            mlm_loss = self.criterion1(scores, kwargs[cfg.PARAM.TARGET_SENT].reshape(-1)) * self.mlm_loss_weight
            #print("scores:",scores)
            #print("loss:",loss)
            #print("mlm loss:",mlm_loss)
            ## MLM分支
            l1 = loss.clone()
            l2 = (self.p*mlm_loss).clone()
            loss = l1 + l2
            
            loss_info = {'CrossEntropy Loss': loss.item()}
            ###################——————————————————————————————#######################
        else:
            """
